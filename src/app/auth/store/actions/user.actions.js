...
export function addCardFirebaseAuthUser(dispatch) {
    const currentUser = firebase.auth().currentUser;
    const user =
    {
        uid: currentUser.uid,
...
export function setCardData(user) {
    return (dispatch) => {
...
function updateCardData(user) {
    console.log(user)
    firebaseService.updateCardData(user)
        .then(() => {
            store.dispatch(Actions.showMessage({ message: "Card data saved" }));
            store.dispatch({
